...
class LitellmFunctionCallModelConfig(LitellmModelConfig):
    action_regex: str = r"<function=execute_bash>.*?<parameter=command>\n?(.*?)\n?</parameter>"
    """Regex to extract the bash command from the XML function-call format."""
    format_error_template: str = (
        "Please always provide EXACTLY ONE action using the execute_bash function format, "
        "found {{actions|length}} actions."
    )
    """Template used when the LM's output is not in the expected format."""


class LitellmFunctionCallModel(LitellmModel):
    def __init__(self, **kwargs):
        super().__init__(config_class=LitellmFunctionCallModelConfig, **kwargs)
